...
epoll_reactor::epoll_reactor(asio::execution_context& ctx)
  : execution_context_service_base<epoll_reactor>(ctx),
    ...
    mutex_(ASIO_CONCURRENCY_HINT_IS_LOCKING(REACTOR_REGISTRATION, scheduler_.concurrency_hint())),
    interrupter_(),
...
void epoll_reactor::notify_fork(asio::execution_context::fork_event fork_ev)
{
    if (fork_ev == asio::execution_context::fork_child)
    {
        ...
        //EPOLLIN事件则只有当对端有数据写入时才会触发，所以触发一次后需要不断读取所有数据直到读完EAGAIN.
        //EPOLLET edge-triggered ：边缘触发
        //EPOLLLT Level-triggered ：水平触发，缺省模式
        ...
        for (descriptor_state* state = registered_descriptors_.first(); state != 0; state = state->next_)
        {
            ...
            if (result != 0)
            {
                asio::error_code ec(errno, asio::error::get_system_category()); asio::detail::throw_error(ec, "epoll re-registration");
            }
        }
    }
}
...
{
    std::cout << "在 epoll_reactor的方法init_task 中调用 scheduler的init_task方法。" << std::endl;
...
{
    std::cout << "执行epoll_reactor的start_op方法。" << std::endl;
    std::cout << "参数：" << std::endl;
    std::cout << "op_type : " << op_type << std::endl;
    std::cout << "socket_type : " << descriptor << std::endl;
    std::cout << "\n";

    if (!descriptor_data)
    {
        std::cout << "descriptor_data 不为空， bad_descriptor." << std::endl;
    ...
    if (descriptor_data->op_queue_[op_type].empty())
    {
        ...
        else
        {
            ...
            std::cout << "添加descriptor到epoll." << std::endl;
    ...
    std::cout << "调用 scheduler的work_started()方法。" << std::endl;
...
{
    std::cout << "进入到epoll_reactor的run方法。" << std::endl;
    ...
    for (int i = 0; i < num_events; ++i)
    {
        ...
        {
            // Ignore.
            std::cout << "触发　interrupter　事件，忽略。" << std::endl;
        }
# if defined(ASIO_HAS_TIMERFD)
        else if (ptr == &timer_fd_)
        {
            // Ignore.
            std::cout << "触发　timer 事件, 忽略。" << std::endl;
        }
# endif // defined(ASIO_HAS_TIMERFD)
        else
        {
            ...
            //ptr 用户自定义数据，(void*)
            ASIO_HANDLER_REACTOR_EVENTS((context(),  reinterpret_cast<uintmax_t>(ptr), event_mask));
        }
    }
    ...
    std::cout << "事件个数：" << num_events << std::endl;
    for (int i = 0; i < num_events; ++i)
    {
        void* ptr = events[i].data.ptr;
        if (ptr == &interrupter_)
    ...
    std::cout << "------------epoll_reactor的run方法完毕----------" << std::endl;
}
...
void epoll_reactor::descriptor_state::do_complete(void* owner, 
                                                  operation* base,
                                                  const asio::error_code& ec, 
                                                  std::size_t bytes_transferred)
{
    std::cout << "epoll_reactor的do_complete方法。" << std::endl;
